...
<body>
    ...
    if (isset($_POST['cancel'])) {
        ...
        $output .= '<p>Your Order, ' . $id . ' has been rejected deu to some unavaoidable reason.</p>';
        ...
        $body = $output;
        $subject = "Order Cancelation - apsitcanteenwala.com";
    ...
    if (isset($_POST['complete'])) {
        ...
        $output = '<p>Dear user,</p>';
        $output .= '<p>Your Order, ' . $id . ' has been completed and ready to be collected.</p>';
        $output .= '<p>-------------------------------------------------------------</p>';
        $output .= '<div class="order complete">
        ...
        $output .= 'APSIT Canteen Team</p>';
        $body = $output;
        $subject = "Order Completion - apsitcanteenwala.com";
        $receiver = $row['user_id'] . '@apsit.edu.in';
        $sender = "From:20102164.meghasoni@gmail.com";
        $sender .= "MIME-Version: 1.0\r\n";
        $sender .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
    ...
    $sql = "select * from order_today where `status` = '1' order by `time` DESC;";
    $result = mysqli_query($conn, $sql);
    while ($row = $result->fetch_assoc()) {
